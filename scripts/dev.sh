...
# binaries, then runs api + gateway + worker + web in this one terminal. Every
# line is prefixed with a colored service tag and scrolls together. Ctrl+C stops
...
# --- colors / labels -------------------------------------------------------
NC=$'\033[0m'
C_DEV=$'\033[1;34m'; C_API=$'\033[36m'; C_GW=$'\033[35m'
...
cleanup() {
  ...
  pkill -f 'bin/apage-gateway' 2>/dev/null
...
export DATABASE_URL REDIS_URL S3_ENDPOINT S3_BUCKET S3_ACCESS_KEY S3_SECRET_KEY \
       APP_BASE_DOMAIN GATEWAY_INTERNAL_URL SESSION_SECRET
...
if ! go build -o bin/apage-api ./cmd/api \
   && go build -o bin/apage-gateway ./cmd/gateway \
   && go build -o bin/apage-worker ./cmd/worker \
   && go build -o bin/apage-agent ./cmd/agent; then
  err "go build failed."
  exit 1
fi

# Clear any stale instances holding the ports.
pkill -f 'bin/apage-(api|gateway|worker)' 2>/dev/null || true
sleep 0.3

# --- 3. app services -------------------------------------------------------
log "starting services — Ctrl+C to stop all."
log "  api  http://localhost:8080   gateway ws://localhost:8090   web http://localhost:${WEB_PORT}"
stream "API"    "$C_API" ./bin/apage-api
stream "GATEWAY" "$C_GW"  ./bin/apage-gateway
